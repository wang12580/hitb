defmodule Edit.PatientService do
    ...
    defp patient_test() do
        content = Repo.all(from p in Cda, select: p.content)
        names =  Repo.all(from p in Cda, select: [p.name, p.username])
        Enum.each(names, fn x -> 
            # IO.inspect x
            [namea | usernamea] = x
            cda = Repo.get_by(Cda, name: namea, username: List.to_string(usernamea))
            result =  Map.drop(cda, [:__meta__, :__struct__])
            IO.inspect result.content
            Enum.each(String.split(result.content, ","), fn y -> 
                b = String.split(y, " ")
                [header | all] = b
                if header == "姓名" do
                    cda
                    |>Cda.changeset(%{patient: List.to_string(all)})
                    |>Repo.update
                end
            end)
        end)
        Enum.each(content, fn x -> 
            a = String.split(x, ",")
            Enum.each(a, fn y -> 
                b = String.split(y, " ")
                [header | all] = b
                if header == "姓名" do
                    body = %{"name" => List.to_string(all)}
                    IO.inspect body
                    %Patient{}
                    |> Patient.changeset(body)
                    |> Repo.insert()
                end
            end)
        end)
